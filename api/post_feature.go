...
func (s *Server) deleteQouteRetweet(arg *QouteRetweetPostRequest, c echo.Context, post db.PostFeature, deletePostid int64) error {
	...
	err = s.store.DeletePostFeature(c.Request().Context(), deletePostid)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, err.Error())
	}
	err = s.store.DeletePost(c.Request().Context(), deletePostid)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, err.Error())
	}
...
func (s *Server) createRetweetPost(param *QouteRetweetPostRequest, c echo.Context) (db.Post, db.PostFeature, error) {
